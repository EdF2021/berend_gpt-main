...
def is_file_valid(file: File) -> bool:
    if (
        ...
        st.error("Kan het document niet lezen. Is het wel een juist formaat? ")
        logger.error("Kan het document niet lezen")
        return False
    return True


def display_file_read_error(e: Exception, file_name: str) -> NoReturn:
    st.error("Kan het document niet lezen.. Is het niet corrupted of encrypted")
